...
if __name__ == '__main__':
    ref_name, weights, insertions, deletions, l_clip_starts, r_clip_starts = kindel.parse_records('tests/HCV_AVU_AB_1.12345.R12.sub.bam')
    cov = coverage(weights)
    cov_smoothed = pd.Series(cov).rolling(window=10).mean().tolist()
    plot(cov_smoothed, l_clip_starts, r_clip_starts)