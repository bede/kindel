...
def parse_records(sam_path, ref_max_len=100000):
    with open(sam_path, 'r') as sam_fh:
        ...
        for pos, weight in enumerate(weights[1:]):
            if not max(weight.values()):
                break
        weights, insertions, deletions = weights[:pos], insertions[:pos], deletions[:pos]
...
if __name__ == '__main__':
    ...
    sam_path = sys.argv[1]
    weights, insertions, deletions = parse_records(sam_path)
    consensus = consensus_sequence(weights, insertions, deletions)
    
    consensus_record = SeqRecord(Seq(consensus), id='cns', description='')
